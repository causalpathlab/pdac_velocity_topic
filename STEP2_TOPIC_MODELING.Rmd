...
load("/Users/samtoussi/Desktop/yppQC/fit10DE_GWASandBG")
...
gplots::heatmap.2(as.matrix(rankedLFC[1:15,]),dendrogram = "none",Rowv = NA,Colv = NA,labRow = F ,srtCol = 0,trace = "none", density = "none", col=myPalette, cellnote =as.matrix(rankedGenes[1:15,]), notecex = 0.5, notecol = "white", colsep=1:15,sepcolor="white",sepwidth=c(0.05,0.05), key.title  <-  "Relative Gene Ratio Value", key.xlab ="Relative Gene Ratio Value",na.color = "black", breaks = myColors)


myColors1  <-  c(seq(0,2.9,length=100),seq(3,25,length=100))
myPalette1 <- colorRampPalette(c("black", "green"))(n = 199)

gplots::heatmap.2(matrix(10,2,2),dendrogram = "none",Rowv = NA,Colv = NA,labRow = F ,labCol = NA,srtCol = 0,trace = "none", density = "none", col=myPalette1, notecex = 0.5, notecol = "white", key.xlab ="u/s Ratio LFC",na.color = "black", breaks = myColors1, keysize = 16 )

myColors2  <-  c(seq(0,2.9,length=100),seq(3,25,length=100))
myPalette2 <- colorRampPalette(c("black", "red"))(n = 199)

gplots::heatmap.2(matrix(10,2,2),dendrogram = "none",Rowv = NA,Colv = NA,labRow = F ,labCol = NA,srtCol = 0,trace = "none", density = "none", col <- myPalette2, notecex = 0.5, notecol = "white", key.xlab ="u/s Ratio LFC",na.color = "black", breaks = myColors2, keysize = 16 )



plot(rep(0,199),col=myPalette[seq(0,199,10)],pch=19,cex=2)
